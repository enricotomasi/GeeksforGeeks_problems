...
class Solution
{
  public:
    ...
    void ric (Node *root, vector<int> &mappa)
    {
        if (root == NULL) return;
        
        ric(root->left, mappa);
        mappa.push_back(root->data);
        ric(root->right, mappa);
    }
    
    void ric2(Node* root, vector<int> &mappa)
    {
        if (root == NULL) return;
        
        ric2(root->left, mappa);
        root->data = mappa[0];
        mappa.erase(mappa.begin());
        ric2(root->right, mappa);
    }
    ...
    Node *binaryTreeToBST (Node *root)
    {
        ...
        /*
            ...
            How about storing the node values of the given binary tree into an array and then sorting the values?
