...
/*
    You simply need to find the Height of the given Binary Tree. 
    The only difficulty is that how to know that you have reached a leaf node so that you do not recur further.
    Can you think of some property of a doubly linked list for knowing the same?
...
int findTreeHeight(Node* root)
{
    ...
    if (root->left && root->right && 
        root->left->right == root && root->right->left == root) return 1;
